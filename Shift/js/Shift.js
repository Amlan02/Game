...
function check() {
  ...
  if(containers[1].children.length > 0) {
...
let statue = document.querySelectorAll('.statue');

Array.from(statue).forEach(value => {
  value.addEventListener('mouseover', function(mouseover) {
    mouseover.srcElement.children[0].style.visibility= 'visible';
    mouseover.preventDefault;
  });

  value.addEventListener('mouseout', function(mouseout) {
    mouseout.srcElement.children[0].style.visibility = 'hidden';
